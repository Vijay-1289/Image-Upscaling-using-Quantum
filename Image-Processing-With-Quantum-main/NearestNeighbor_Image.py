...
# Determine upscale target size using NEAREST NEIGHBOR
...
# Nearest neighbor resizing instead of bilinear
zoom_factors = (upscale_shape[0] // original_shape[0], upscale_shape[1] // original_shape[1])
input_matrix = original_img.repeat(zoom_factors[0], axis=0).repeat(zoom_factors[1], axis=1)
...
axs[1].set_title(f'Nearest Neighbor Upscaled Image {input_matrix.shape[0]}x{input_matrix.shape[1]}')
axs[1].set_xlabel("x")
axs[1].set_ylabel("y")
